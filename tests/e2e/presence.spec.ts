...
test.describe('Presence submission', () => {
  test('user can submit presence and view locked state', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Today I am coming...' })).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: 'Update my presence' })).toBeVisible({ timeout: 10000 });
